(function initAdminHomeBannerPage() {
  const section = document.getElementById('adminSectionHomeBanner');
  const layout = document.getElementById('adminHomeBannerLayout');
  const pageBackBtn = document.getElementById('adminHomeBannerPageBack');
  const countEl = document.getElementById('adminHomeBannerCount');
  const listEl = document.getElementById('adminHomeBannerList');
  const formEl = document.getElementById('adminHomeBannerForm');
  const formTitle = document.getElementById('adminHomeBannerFormTitle');
  const addBtn = document.getElementById('adminHomeBannerAdd');
  const saveBtn = document.getElementById('adminHomeBannerSaveBtn');
  const cancelBtn = document.getElementById('adminHomeBannerCancelBtn');
  const deleteBtn = document.getElementById('adminHomeBannerDeleteBtn');
  const pickImageBtn = document.getElementById('adminHomeBannerPickImage');
  const removeImageBtn = document.getElementById('adminHomeBannerRemoveImage');
  const imageInput = document.getElementById('adminHomeBannerImageInput');
  const editPreview = document.getElementById('adminHomeBannerEditPreview');
  const editPreviewImg = document.getElementById('adminHomeBannerEditPreviewImg');
  const dragHint = document.getElementById('adminHomeBannerDragHint');
  const fitControls = document.getElementById('adminHomeBannerFitControls');
  const scaleInput = document.getElementById('adminHomeBannerScale');
  const scaleValue = document.getElementById('adminHomeBannerScaleVal');
  const saveFitBtn = document.getElementById('adminHomeBannerSaveFit');
  const successEl = document.getElementById('adminHeroSuccess');
  const warningEl = document.getElementById('adminHeroWarning');

  if (!section || !layout || !listEl || !formEl || !window.HeroBanners) return;
  ...
  function renderList() {
    ...
    if (!entries.length) {
      listEl.innerHTML = `
        <li class="admin-fitness-empty admin-home-banner-empty">
          <span class="admin-fitness-empty-icon" aria-hidden="true">🏠</span>
          <strong>Баннеров пока нет</strong>
          <p>Добавьте первый hero-баннер — он появится на главной странице сайта</p>
        </li>`;
      return;
    }
    ...
    listEl.innerHTML = entries.map(({ index, slide }) => {
      ...
      return `
        <li class="admin-fitness-product-item admin-home-banner-item${editingIndex === index ? ' is-active' : ''}" data-select-index="${index}" role="button" tabindex="0" aria-label="Редактировать баннер ${index + 1}">
          ${imageUrl
            ? `<button type="button" class="admin-home-banner-thumb-btn" data-zoom-src="${escapeHtml(imageUrl)}" data-zoom-label="Баннер ${index + 1}" aria-label="Увеличить баннер ${index + 1}">
                <span class="admin-fitness-product-thumb admin-home-banner-thumb admin-home-banner-thumb-bg" style="${thumbStyle}" role="img" aria-hidden="true"></span>
              </button>`
            : '<div class="admin-fitness-product-thumb admin-fitness-product-thumb--placeholder admin-home-banner-thumb--placeholder"><span>?</span></div>'}
          <div class="admin-fitness-product-info">
            <strong>Баннер ${index + 1}</strong>
            <p>${escapeHtml(sizeLabel)}</p>
            <span class="admin-home-banner-pick-meta">Hero-слайд</span>
          </div>
  ...
  function openCreateForm() {
    if (getNextFreeIndex(slideData) < 0) {
      showToast(`Максимум ${HERO_MAX_SLIDES} баннеров`, 'error');
      return;
    }
    ...
    try {
      if (editingIndex === null) {
        ...
        if (!isRatioOk(width, height)) {
          showMessage(
            warningEl,
            `Загружено ${width}×${height} px — рекомендуется ${HERO_RECOMMENDED.width}×${HERO_RECOMMENDED.height} px`
          );
        }
  ...
  async function openPage() {
    if (typeof window.showAdminSection === 'function') {
      window.showAdminSection('homeBanner');
    }
    resetForm();
    await fetchSlides();
  }
  ...
  window.addEventListener('hero-banners-updated', fetchSlides);

  window.openAdminHomeBannerModal = openPage;
