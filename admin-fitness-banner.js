(function initAdminFitnessBannerPage() {
  const section = document.getElementById('adminSectionFitnessBanner');
  const layout = document.getElementById('adminFitnessBannerLayout');
  const pageBackBtn = document.getElementById('adminFitnessBannerPageBack');
  const countEl = document.getElementById('adminFitnessBannerCount');
  const listEl = document.getElementById('adminFitnessBannerList');
  const formEl = document.getElementById('adminFitnessBannerForm');
  const formTitle = document.getElementById('adminFitnessBannerFormTitle');
  const addBtn = document.getElementById('adminFitnessBannerAdd');
  const saveBtn = document.getElementById('adminFitnessBannerSaveBtn');
  const cancelBtn = document.getElementById('adminFitnessBannerCancelBtn');
  const deleteBtn = document.getElementById('adminFitnessBannerDeleteBtn');
  const pickImageBtn = document.getElementById('adminFitnessBannerPickImage');
  const removeImageBtn = document.getElementById('adminFitnessBannerRemoveImage');
  const imageInput = document.getElementById('adminFitnessBannerImageInput');
  const editPreview = document.getElementById('adminFitnessBannerEditPreview');
  const editPreviewImg = document.getElementById('adminFitnessBannerEditPreviewImg');
  const dragHint = document.getElementById('adminFitnessBannerDragHint');
  const fitControls = document.getElementById('adminFitnessBannerFitControls');
  const scaleInput = document.getElementById('adminFitnessBannerScale');
  const scaleValue = document.getElementById('adminFitnessBannerScaleVal');
  const saveFitBtn = document.getElementById('adminFitnessBannerSaveFit');
  const successEl = document.getElementById('adminFitnessBannerSuccess');
  const warningEl = document.getElementById('adminFitnessBannerWarning');

  if (!section || !layout || !listEl || !formEl || !window.FitnessBanners) return;
  ...
  function renderList() {
    ...
    if (!entries.length) {
      listEl.innerHTML = `
        <li class="admin-fitness-empty admin-fitness-banner-empty">
          <span class="admin-fitness-empty-icon" aria-hidden="true">💪</span>
          <strong>Баннеров пока нет</strong>
          <p>Добавьте первый hero-баннер — он появится в разделе «Фитнес-индустрия» на сайте</p>
        </li>`;
      return;
    }
    ...
    listEl.innerHTML = entries.map(({ index, slide }) => {
      ...
      return `
        <li class="admin-fitness-product-item admin-fitness-banner-item${editingIndex === index ? ' is-active' : ''}" data-select-index="${index}" role="button" tabindex="0" aria-label="Редактировать баннер ${index + 1}">
          ${imageUrl
            ? `<button type="button" class="admin-fitness-banner-thumb-btn" data-zoom-src="${escapeHtml(imageUrl)}" data-zoom-label="Баннер ${index + 1}" aria-label="Увеличить баннер ${index + 1}">
                <span class="admin-fitness-product-thumb admin-fitness-banner-thumb admin-fitness-banner-thumb-bg" style="${thumbStyle}" role="img" aria-hidden="true"></span>
              </button>`
            : '<div class="admin-fitness-product-thumb admin-fitness-product-thumb--placeholder admin-fitness-banner-thumb--placeholder"><span>?</span></div>'}
          <div class="admin-fitness-product-info">
            <strong>Баннер ${index + 1}</strong>
            <p>${escapeHtml(sizeLabel)}</p>
            <span class="admin-fitness-banner-pick-meta">Hero-слайд</span>
          </div>
  ...
  function openCreateForm() {
    if (getNextFreeIndex(slideData) < 0) {
      showToast(`Максимум ${FITNESS_MAX_SLIDES} баннеров`, 'error');
      return;
    }
    ...
    try {
      if (editingIndex === null) {
        ...
        if (!isRatioOk(width, height)) {
          showMessage(
            warningEl,
            `Загружено ${width}×${height} px — рекомендуется ${FITNESS_RECOMMENDED.width}×${FITNESS_RECOMMENDED.height} px`
          );
        }
  ...
  async function openPage() {
    if (typeof window.showAdminSection === 'function') {
      window.showAdminSection('fitnessBanner');
    }
    resetForm();
    await fetchSlides();
  }
  ...
  window.addEventListener('fitness-banners-updated', fetchSlides);

  window.openAdminFitnessBannerPage = openPage;
  window.openFitnessBannerEditor = openPage;
